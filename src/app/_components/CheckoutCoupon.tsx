    ...
    <div className="mt-6 rounded-xl border border-black/10 p-4">
      ...
      <div className="mt-2 flex gap-2">
        <input
          ...
          className="min-w-0 flex-1 rounded-lg border border-black/15 bg-white px-3 py-2 text-xs"
        />
        <button
          type="button"
          disabled={disabled || !draft.trim()}
          onClick={() => onChange(draft.trim())}
          className="rounded-lg bg-[#20211e] px-3 py-2 text-xs text-white disabled:opacity-40"
        >
          Apply
        </button>
      </div>
