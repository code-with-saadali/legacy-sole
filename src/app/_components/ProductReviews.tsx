...
export default function ProductReviews({ slug }: { slug: string }) {
  ...
  const load = useCallback(async () => {
    ...
    if (error) setError("Reviews could not be loaded. Please retry.");
    else {
      setReviews(data ?? []);
      setError("");
    }
  ...
  return (
    ...
    >
      <p className="text-[10px] uppercase tracking-[0.22em] text-[#b66b4d]">
        Worn. Walked. Reviewed.
      </p>
      <div className="mt-3 flex flex-wrap items-end justify-between gap-4">
        <h2 id="reviews-title" className="text-4xl text-[#20211e] sm:text-5xl">
          From our community.
        </h2>
        <p className="max-w-sm text-sm leading-6 text-black/50">
          Real opinions on everyday comfort, fit and style.
        </p>
      </div>
        ...
        <div className="min-w-0">
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {filtered.slice(0, limit).map((review) => (
            ...
            <div className="rounded-2xl border border-dashed border-black/15 px-6 py-12 text-center">
              ...
              <p className="mt-3 text-sm text-black/50">
                Share a few details to help someone find their next pair.
              </p>
            </div>
          )}
          {filtered.length > limit && (
            <button
              ...
              className="mt-5 rounded-full border border-black/20 px-5 py-3 text-sm"
            >
              Show more reviews
            </button>
          )}
      ...
      <div className="mt-8">
        <ReviewForm key={slug} slug={slug} onSubmitted={() => void load()} />
      </div>
    </section>
  );
}
