...
export default function ReviewModeration() {
  const { rows, error, setError, loading, reload } =
    useAdminRows<Review>("reviews");
        ...
        {visible.map((review) => (
          <div key={review.id}>
            ...
            <button
              ...
              className="mt-3 rounded-full border border-black/15 px-4 py-2 text-xs"
            >
              {review.approved ? "Hide review" : "Approve & publish"}
            </button>
          </div>
        ))}
        {!visible.length && (
